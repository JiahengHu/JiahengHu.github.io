...
We want $(w^Tx^{(i)} + b)$ to be a large positive number if label is positive or large negative number if label is negative. Thus, it means that **functional margin should be positive to be correct.** We need to normalize w and b since magnitude of w and b should not affect the scale of functional margin. 
