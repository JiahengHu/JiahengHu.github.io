...
$$\begin{cases} f(w)  \text{if w satisfy primal constraints \\ \infty  \text{otherwise} \\ \end{cases}$$

To match to our primal problem, w define the min problem as:

$$\min_w \theta_{\mathcal{P}}(w) = \min_w \max_{\alpha,\beta:\alpha_i\geq 0} \mathcal{L}(w,\alpha,\beta)$$

This is the same as the primal problem if all constrain are satisfied. We define the value of primal problem to be: $p^* = \min_w \theta_{\mathcal{P}(w)$. Then, we define:
