...
We want $(w^Tx^{(i)} + b)$ to be a large positive number if label is positive or large negative number if label is negative. Thus, it means that **functional margin should be positive to be correct.** We need to normalize w and b **with respect to the norm of w** since magnitude of w and b should not affect the scale of functional margin. Furthermore, we denote the function margin as:

$$\overset{\wedge}{\gamma} = \min_{i=1,\dots,m} \overset{\wedge}{\gamma^{(i)}} $$

where m is the number of training samples. A figure for example can be shown:
